...
const CHART_TYPES = {
    GRID: gettext("Tabelle"),
    MAP: gettext("Heatmap"),
    PIE: gettext("Kreisdiagramm"),
    BAR: gettext("Balkendiagramm")
};
const TITLES = {
    // Bar/Pie chart titles
    PIE: gettext("Relative Häufigkeiten aller Zahlen (%)."),
    BAR: gettext("Absolut Häufigkeiten aller Zahlen."),
    RESULTS: gettext("Ihre Genauigkeit (%) beim Finden der häufigsten Zahl.")
};
const COLOURS = [
    // Pie chart colours
    "#3e95cd",
...
function runGuessCheck() {
    ...
    if (dataVis.userResponses.length != dataVis.typesAndSolutions.length) {
        // At this point the two should always be equal
        console.log(
            "ERROR: Number of responses does not match number of solutions."
        );
    }
...
function getNextChart() {
    ...
    } /*(dataVis.renderChart == CHART_TYPES.BAR)*/ else {
        dataVis.numberOfDataPoints = Math.min(
            MAX_DATA_POINTS,
            dataVis.numberOfDataPoints * 2
        ); // More data points for the next cycle
        dataVis.renderChart = CHART_TYPES.GRID;
    }
}
        ...
        for (var x = 0; x < numColumns; x++) {
            if (dataVis.renderChart == CHART_TYPES.MAP) {
                html +=
                    ...
                    HEATMAP[dataVis.currentDataSet[i]] +
                    ";'>" +
                    dataVis.currentDataSet[i] +
                    "</td>\n";
            } else {
...
function buildPieChart() {
    ...
    for (var i in dataVis.dataFrequencies) {
        if (dataVis.dataFrequencies[i]) {
            ...
            dataPoints.push(
                Math.round(
                    (dataVis.dataFrequencies[i] /
                        dataVis.currentDataSet.length) *
                        100
                )
            ); // Percentage of total
        }
...
function getFrequencies() {
    ...
    for (var v in localDataSet) {
        ...
        if (frequencies[localDataSet[v]] == max) {
            // More than one current mode
            modes.push(localDataSet[v]);
        } else if (frequencies[localDataSet[v]] > max) {
            // New (singular) mode
    ...
    if (modes.length > 1) {
        ...
        while (localDataSet[randomValue] == mode) {
            // Find a random not-best-mode number
            randomValue++;
        }
